...
export const LoginPage: React.FC<LoginPageProps> = ({ onOpenKiosk }) => {
  const { login, loginSantri, settings, users, students } = useLibrary();
  
  const [loginRole, setLoginRole] = useState<'santri' | 'staff'>('santri');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (loginRole === 'santri') {
      if (!identity.trim()) {
        setErrorMessage('Silakan masukkan NIS Santri.');
        return;
      }
      if (!password.trim()) {
        setErrorMessage('Silakan masukkan kata sandi (kode RFID).');
        return;
      }
      ...
      try {
        const res = await loginSantri(identity, password);
        if (!res.success) {
          setErrorMessage(res.message);
        }
      } catch (err: any) {
        setErrorMessage(err?.message || 'NIS atau password salah.');
      } finally {
        setIsLoading(false);
      }
      return;
  ...
  const handleQuickLogin = (demoUsername: string, demoPass: string, role: 'santri' | 'staff' = 'staff') => {
    setLoginRole(role);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-emerald-950 to-slate-900 text-slate-100 flex flex-col justify-center items-center px-4 py-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 lg:grid-cols-12 gap-6 z-10">
        ...
        <div className="lg:col-span-7 bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-black/50 relative flex flex-col justify-between">
          <div>
            {/* Role Switcher Tabs */}
            <div className="flex rounded-2xl bg-slate-950/80 p-1.5 border border-slate-800/90 mb-6">
              <button
                type="button"
                id="tab-login-santri"
                onClick={() => {
              ...
              {loginRole === 'santri' ? (
                <>
                  <span className="text-xs font-semibold uppercase tracking-wider text-emerald-400 flex items-center gap-1.5 mb-1.5">
                    <GraduationCap className="w-4 h-4" />
                    Portal Santri Perpustakaan
                  </span>
                  <h2 className="text-2xl font-bold text-white">
                    Masuk Akun Santri
                  </h2>
                  <p className="text-xs text-slate-400 mt-1">
                    Gunakan NIS sebagai Username dan Kode Kartu RFID sebagai Kata Sandi.
                  </p>
                </>
              ) : (
                <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1.5">
                  {loginRole === 'santri' ? 'NIS (Nomor Induk Santri)' : 'Email atau Username Supabase'}
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-500">
                    {loginRole === 'santri' ? <GraduationCap className="w-4 h-4" /> : <User className="w-4 h-4" />}
                  </div>
                  <input
                    ...
                    placeholder={loginRole === 'santri' ? 'Contoh: 20260015' : 'Contoh: ustadz@darululum.sch.id atau admin'}
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    <span>{loginRole === 'santri' ? 'Masuk ke Dashboard Santri' : 'Masuk ke Dashboard'}</span>
                    <ArrowRight className="w-4 h-4" />
                  </>
                )}
              </button>

              {loginRole === 'staff' && isSupabaseConfigured && (
        ...
        <div className="lg:col-span-5 flex flex-col justify-between space-y-4">
          ...
          <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800/90 rounded-3xl p-6 shadow-xl">
            ...
            <div className="space-y-2.5">
              {/* Santri Quick Accounts */}
              {students.length > 0 && (
                <div className="space-y-2">
                  <div className="text-[11px] font-semibold text-emerald-400 uppercase tracking-wider flex items-center gap-1.5 pt-1">
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-white group-hover:text-emerald-200 truncate max-w-[140px]">
                              {s.name}
                            </span>
                          </div>
              ...
              <div className="text-[11px] font-semibold text-amber-400 uppercase tracking-wider flex items-center gap-1.5 pt-2">
                <ShieldCheck className="w-3.5 h-3.5" />
                <span>Akun Administrator & Petugas</span>
              </div>
              ...
              <button
                ...
                type="button"
                onClick={() => handleQuickLogin('admin', 'admin123', 'staff')}
              ...
              <button
                ...
                type="button"
                onClick={() => handleQuickLogin('fatimah', 'staff123', 'staff')}
