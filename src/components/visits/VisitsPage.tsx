...
export const VisitsPage: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
              {paginatedVisits.length === 0 ? (
                ...
                paginatedVisits.map((visit, idx) => {
                  ...
                  return (
                    <tr key={visit.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2.5">
                          {student?.photo_url ? (
                            <img src={student.photo_url} alt={student.name} className="w-8 h-8 rounded-lg object-cover ring-1 ring-slate-200 dark:ring-slate-700" />
                          ) : (
                            <div className="w-8 h-8 rounded-lg bg-slate-200 dark:bg-slate-700 flex items-center justify-center font-bold text-slate-600 dark:text-slate-300 ring-1 ring-slate-200 dark:ring-slate-700">
                              {student?.name?.charAt(0) || '?'}
                            </div>
                          )}
                          <span className="font-bold text-slate-900 dark:text-white">{student?.name || 'Santri'}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4 font-mono text-slate-800 dark:text-slate-200">{checkInTime} WIB</td>
                      <td className="py-3 px-4 font-mono text-slate-600 dark:text-slate-400">{checkOutTime} {visit.check_out ? 'WIB' : ''}</td>
                      ...
                      <td className="py-3 px-4">
                        {isInside ? (
                          ...
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-[10px]">
                      ...
                      <td className="py-3 px-4 text-center">
                        {isInside && (
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg bg-rose-50 dark:bg-rose-950/60 hover:bg-rose-100 text-rose-700 dark:text-rose-300 font-semibold text-[11px] border border-rose-200 dark:border-rose-800 transition-colors cursor-pointer"
                          >
                            Check-Out
                          </button>
                        )}
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="p-1 rounded-md bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-30 cursor-pointer text-slate-700 dark:text-slate-300"
              >
      ...
      {/* Visits Mobile Cards List (Mobile Smartphone View) */}
      <div className="sm:hidden space-y-3">
        {paginatedVisits.length === 0 ? (
          <div className="p-8 text-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 text-slate-400">
            ...
            <p className="font-semibold text-sm">Tidak ada riwayat kunjungan</p>
            <p className="text-xs text-slate-400 mt-1">Coba sesuaikan filter pencarian.</p>
          </div>
        ) : (
          paginatedVisits.map((visit, idx) => {
            const student = studentsMap.get(visit.student_id);
            ...
            return (
              <div
                key={`mobile-visit-${visit.id}-${idx}`}
                className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-xs space-y-2.5"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2.5">
                    {student?.photo_url ? (
                      <img src={student.photo_url} alt={student.name} className="w-10 h-10 rounded-xl object-cover ring-1 ring-slate-200 dark:ring-slate-700 shrink-0" />
                    ) : (
                      <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-slate-600 dark:text-slate-300 ring-1 ring-slate-200 dark:ring-slate-700 shrink-0">
                        {student?.name?.charAt(0) || '?'}
                      </div>
                    )}
                    <div>
                  ...
                  {isInside ? (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 font-bold text-[10px] border border-emerald-200 dark:border-emerald-800 shrink-0">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" />
                      DI DALAM
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-semibold text-[10px] shrink-0">
                      SELESAI
                    </span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800">
                  <div>
                    ...
                    <span className="font-mono font-medium text-slate-800 dark:text-slate-200">{checkInTime}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase block">Keluar</span>
                    <span className="font-mono font-medium text-slate-800 dark:text-slate-200">{checkOutTime}</span>
                  </div>
                  <div className="col-span-2 pt-1 border-t border-slate-200/60 dark:border-slate-700 flex items-center justify-between text-[11px]">
                    <span className="text-slate-500 dark:text-slate-400">Durasi:</span>
                    <span className="font-bold text-slate-800 dark:text-slate-200">
                      {isInside ? 'Sedang berjalan' : (visit.duration_minutes ? `${visit.duration_minutes} menit` : '-')}
                    </span>
                  </div>
                </div>
                ...
                {isInside && (
                  <button
                    onClick={() => manualCheckOut(visit.id)}
                    className="w-full py-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/60 hover:bg-rose-100 text-rose-700 dark:text-rose-300 font-bold text-xs border border-rose-200 dark:border-rose-800 active:scale-98 transition-all"
                  >
                    Check-Out Manual
                  </button>
                )}
        ...
        {/* Mobile Pagination */}
        {totalPages > 1 && (
          <div className="flex items-center justify-between p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs">
            <button
              disabled={page <= 1}
              onClick={() => setPage(p => p - 1)}
              className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 disabled:opacity-40 text-slate-700 dark:text-slate-300 font-semibold"
            >
              Sebelumnya
            </button>
